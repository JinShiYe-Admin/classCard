var events = (function(mod) {
	...
	mod.setCookie = function(key,model){
		$.cookie(key, JSON.stringify(model), { expires: 365 });
	}
	mod.getCookie = function(key){
	 ...
	 * 设置cookie中的值
	 */
	mod.setCookie = function(c_name, value) {
		var exdate = new Date();
		var expiredays = 300000;
	...
	mod.getCookie = function(c_name) {
		console.log('c_name0:'+c_name);
		if(document.cookie.length > 0) {
			c_start = document.cookie.indexOf(c_name + "=");
			console.log('c_name:'+c_name);
			if(c_start != -1) {
				console.log('c_name1:'+c_name);
